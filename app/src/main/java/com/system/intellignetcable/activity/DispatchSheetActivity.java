...
public class DispatchSheetActivity extends BaseActivity implements AdapterView.OnItemClickListener {
    ...
    @BindView(R.id.show_template_tv)
    TextView mShowTemplateTv;
        ...
        builder.setSingleChoiceItems(items, -1,
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if(which!=-1){
                            mTemplateNameTv.setText(items[which]);
                        }
                        currentTemolatPos = which;
                        mShowTemplateTv.setVisibility(View.VISIBLE);
                    }
                });
    ...
    @OnClick({R.id.template_rl, R.id.show_template_tv})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.template_rl:
                if (templateList.size() > 0) {
                    showTemplateDialog(templateList);
                }
                break;
            case R.id.show_template_tv:
                //TODO 查询模板
                if (templatesBeanList.size() > 0) {
                    int id = templatesBeanList.get(currentTemolatPos).getUserTeamplateId();
